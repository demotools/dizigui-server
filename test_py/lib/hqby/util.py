...
def _execute_sql(sql, values, mode='execute'):
    ...
    dbc = hqby.db.get_conn(_DATABASE_NAME)
...
class HqOrm(object):
    ...
    @classmethod
    def get(cls, id=None, fields=None, **kwargs):
    ...
    def find(cls, args=None, join=None, fields=None, **kwargs):
        ...
        if join:
            ...
            join_sql = join[0]
        sql = 'SELECT %s FROM `%s` %s WHERE %s %s %s' % (list_to_sql(fields), cls._table_name,
    ...
    def dictify(self, fields=None, properties=None):
        ...
        if 'created' in data:
            data['created'] = tools.to_str(data['created'])
        if 'updated' in data:
            data['updated'] = tools.to_str(data['updated'])
        if 'expires' in data:
            data['expires'] = tools.to_str(data['expires'])
        if 'ins_time' in data:
            data['ins_time'] = tools.to_str(data['ins_time'])
        return data
